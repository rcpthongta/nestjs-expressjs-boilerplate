...
export const printf: () => winston.Logform.Format = (): winston.Logform.Format => {
  return winston.format.printf((transformable: TransformableOptions): string => {
    ...
    return `${transformable.label} [${transformable.pid}] ${transformable.rid} - ${transformable.timestamp}   ${transformable.level} [${transformable.context}] ${transformable.message} ${transformable.ms}${stack}`;
  });
};
